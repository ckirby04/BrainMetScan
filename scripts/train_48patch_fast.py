"""
Fast 48-patch training with pre-loading.
Pre-loads all patches into RAM for maximum GPU utilization.

Usage:
    python scripts/train_48patch_fast.py
"""
...
def main():
    # Config
    BATCH_SIZE = 16  # Smaller batch for 48³ patches (more GPU memory per sample)
    EPOCHS = 150
    LR = 0.001
    PATCH_SIZE = (48, 48, 48)
    ...
    print(f"Total RAM: {img_size_mb + mask_size_mb:.1f} MB")
    ...
    best_val_dice = 0
    model_path = model_dir / "tiny_lesion_48patch_best.pth"
    state_path = model_dir / "tiny_lesion_48patch_state.json"
