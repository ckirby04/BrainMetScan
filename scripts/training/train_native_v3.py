"""
Train a single native-geometry base model with v3 improvements:
  1. SmallLesionOptimizedLoss (replaces Dice+Focal+Tversky)
  2. MONAI augmentation pipeline (replaces basic flips/rotations)
  3. Hybrid weighted sampling (oversamples small lesions + difficult cases)

Same LightweightUNet3D architecture as existing base models.

Usage:
    python scripts/training/train_native_v3.py --patch-size 8 --gpu 0
    python scripts/training/train_native_v3.py --patch-size 12 --gpu 0
    python scripts/training/train_native_v3.py --patch-size 24 --gpu 1
    python scripts/training/train_native_v3.py --patch-size 36 --gpu 1
"""
...
# Native data uses t2 (preprocessed_256 has t2.nii.gz not bravo)
...
def train_model(args):
    ...
    model_dir = ROOT / "model" / "native_v3"
    model_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n{'#'*70}")
    print(f"# NATIVE v3 TRAINING (SmallLesionLoss + MONAI aug + weighted sampling)")
    print(f"# Patch: {patch_size}^3 | Epochs: {epochs} | Batch: {batch_size}")
    print(f"# GPU {args.gpu}: {gpu_name} ({gpu_mem:.1f} GB)")
    print(f"# Data: {args.data_dir} (566 cases)")
    print(f"{'#'*70}\n")

    # Build dataset with MONAI augmentation
    # Case-level split first, then load patches
    full_ds = BrainMetDataset(
        data_dir=str(args.data_dir),
        sequences=SEQUENCES,
        patch_size=None,
        target_size=None,  # Native data is already preprocessed to 256^3
        augment=False,
    )
    ...
    train_ds = BrainMetDataset(
        data_dir=str(args.data_dir),
        sequences=SEQUENCES,
        patch_size=PATCH,
        target_size=None,  # Already 256^3
        augment=True,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if is_best:
            ...
            torch.save({
                ...
                "val_loss": val_loss,
                "val_dice": val_dice,
                "dice": val_dice,  # compat with train_stacking.py
                "dice_scores": dice_scores,
                "sensitivity": sensitivity,
                "model_config": MODEL_CONFIG,
                "patch_size": patch_size,
                "training": "v3 (SmallLesionLoss + MONAI aug + weighted sampling)",
            }, best_path)
    ...
    print(f"\n{'='*70}")
    print(f"  {patch_size}^3 v3 COMPLETE in {total_min:.1f} min")
...
def main():
    parser = argparse.ArgumentParser(description="Native v3: improved loss + MONAI aug + weighted sampling")
    ...
    if args.data_dir is None:
        args.data_dir = ROOT / "data" / "preprocessed_256" / "train"
    else:
