"""
nnU-Net 2D training on Dataset001_BrainMets.

Trains nnU-Net in 2D configuration (slice-by-slice) to add diversity to the
ensemble. 2D models process each axial slice independently, giving different
error patterns compared to 3D models — useful for stacking.

2D preprocessed data and plans already exist from nnU-Net planning.
Uses subprocess to launch training (avoids Windows spawn/multiprocessing hangs).
Auto-restarts when epoch times degrade (same workaround as train_multifold.py).

Usage:
    python scripts/train_2d.py                     # Train all 5 folds
    python scripts/train_2d.py --folds 0           # Train fold 0 only
    python scripts/train_2d.py --folds 0 1 2       # Train specific folds
    python scripts/train_2d.py --status             # Check fold status
    python scripts/train_2d.py --eval               # Evaluate completed folds
"""
...
CONFIGURATION = "2d"
TRAINER_DIR = RESULTS_DIR / DATASET_NAME / f"nnUNetTrainer__nnUNetPlans__{CONFIGURATION}"
...
def print_status():
    """Print status of all 2D folds."""
    print("\n" + "=" * 60)
    print("  nnU-Net 2D Training Status")
    print("=" * 60)
    ...
    print()
    if all_complete:
        print("  All 2D folds complete! Ready for ensemble integration.")
    print()
...
def train_fold(fold: int):
    """Train a single 2D fold with auto-restart on degradation."""
    s = get_fold_status(fold)
    ...
    if s["has_final"]:
        print(f"\n  Fold {fold} has checkpoint_final but no validation. Running validation...")
    ...
    while True:
        ...
        if restart_count > 0:
            ...
            print(f"\n{'=' * 60}")
            print(f"  {action} 2D fold {fold}{epoch_info}")
            print(f"  Patch: 224x224, Batch: 65")
...
def evaluate_all():
    """Run evaluate_nnunet.py for completed 2D folds."""
    ...
    if not completed:
        print("  No completed 2D folds to evaluate.")
        return

    print(f"\n  Evaluating {len(completed)} completed 2D fold(s): {completed}")
    trainer = f"nnUNetTrainer__nnUNetPlans__{CONFIGURATION}"
    for fold in completed:
        print(f"\n{'=' * 60}")
        print(f"  Evaluating 2D fold {fold}")
        print(f"{'=' * 60}")
        subprocess.run([
...
def main():
    parser = argparse.ArgumentParser(description="nnU-Net 2D training")
    ...
    # Training mode
    print("\n" + "=" * 60)
    print("  nnU-Net 2D Training Pipeline")
    print("  Configuration: 2d | Patch: 224x224 | Batch: 65")
    print("  Expected: ~2-4 hours per fold (much faster than 3D)")
    print("=" * 60)
    ...
    est_hours = len(folds_to_train) * 3
    print(f"  Estimated total time: ~{est_hours} hours\n")

    for i, fold in enumerate(folds_to_train):
        print(f"\n  [{i + 1}/{len(folds_to_train)}] Training 2D fold {fold}...")
        t0 = time.time()
    ...
    print("\n" + "=" * 60)
    print("  2D Training session complete!")
    print("=" * 60)
    print_status()

    print("  Next steps:")
    print("    1. Evaluate:  python scripts/train_2d.py --eval")
    print("    2. Generate probs: python scripts/nnunet_probs.py --mode val --trainer nnUNetTrainer__nnUNetPlans__2d")
    print("    3. Add to stacking: python scripts/train_stacking.py --include-nnunet --include-2d")
